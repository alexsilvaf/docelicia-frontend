...
export function LandingPage() {
  ...
  useEffect(() => {
    document.title = "Docelícia Doceria & Café | Cafeteria e Confeitaria em Piúma-ES";
  }, []);
  ...
  return (
    ...
    >
      {/* ── Nav — top navigation ── */}
      <header>
        <nav className="sticky top-0 z-40 bg-card/95 backdrop-blur border-b border-border" aria-label="Navegação principal">
          <div className="max-w-6xl mx-auto px-4 lg:px-6 flex items-center justify-between h-12 md:h-14">
            <button onClick={() => navigate("/dashboard")} className="flex items-center gap-1.5 cursor-pointer">
              ...
              <span className="text-foreground font-semibold">Docelícia</span>
            </button>
            ...
            <div className="flex items-center gap-1">
              ...
              <a
                ...
                rel="noopener noreferrer"
                className="p-2 text-muted-foreground hover:text-foreground cursor-pointer transition-colors min-w-[36px] min-h-[36px] flex items-center justify-center"
                aria-label="Siga a Docelícia no Instagram"
              >
      ...
      <main>
        ...
        <section id="inicio" className="relative" aria-label="Apresentação da Docelícia">
          <div className="absolute inset-0 overflow-hidden">
            <ImageWithFallback
              src={HERO_IMG}
              alt="Ambiente acolhedor da Docelícia Doceria e Café em Piúma com doces artesanais e cafés especiais"
          ...
          <div className="relative max-w-6xl mx-auto px-4 lg:px-6 py-16 md:py-28 lg:py-36 flex flex-col items-center text-center md:items-start md:text-left">
            <div className="max-w-lg">
              <span className="text-primary-foreground/80 block mb-1">Cafeteria & Confeitaria em Piúma-ES</span>
              <h1 className="text-primary-foreground mb-3" style={{ textShadow: "0 2px 8px rgba(0,0,0,0.3)" }}>
                Docelícia Doceria &amp; Café
              </h1>
        ...
        {/* ── Quick info bar ── */}
        <section className="bg-card border-b border-border" aria-label="Informações rápidas">
          <div className="max-w-6xl mx-auto px-4 lg:px-6 py-4 md:py-5">
            <div className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
              <div>
                <span className="text-muted-foreground block text-sm">Horário</span>
                <p className="text-card-foreground">
                  <time>Ter a Sex · 15h–20h</time> · <time>Sáb · 14h–18h</time>
                </p>
              </div>
              <div className="hidden md:block w-px h-8 bg-border" aria-hidden="true" />
              <div>
                <span className="text-muted-foreground block text-sm">Endereço</span>
        ...
        {/* ── Highlights — most popular items ── */}
        <section id="destaques" className="bg-background" aria-label="Destaques do cardápio">
          <div className="max-w-6xl mx-auto px-4 lg:px-6 py-8 md:py-14">
            <div className="mb-5 md:mb-8 md:text-center">
              <span className="text-primary block mb-0.5 text-sm font-medium">Destaques</span>
              ...
              {HIGHLIGHTS.map((item) => (
                ...
                >
                  <div className="aspect-[4/3] overflow-hidden">
                    <ImageWithFallback
                      src={item.img}
                      alt={`${item.name} — ${item.desc} por ${item.price} na Docelícia Piúma`}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                    />
                  </div>
                  <div className="p-3 md:p-4">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="text-card-foreground text-base font-medium">{item.name}</h3>
                      <div className="flex gap-0.5" aria-label={`Avaliação: 5 de 5 estrelas`}>
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star key={s} className="w-3 h-3 text-chart-4 fill-chart-4" aria-hidden="true" />
                        ))}
                      </div>
                    </div>
                    <p className="text-muted-foreground text-sm line-clamp-2 mb-2">{item.desc}</p>
                    <p className="text-primary font-medium">{item.price}</p>
                  </div>
                </article>
              ))}
            </div>
            <div className="flex justify-center gap-1.5 mt-3 md:hidden" aria-hidden="true">
              {HIGHLIGHTS.map((_, i) => (
        ...
        {/* ── Cardápio ── */}
        <section id="cardapio" className="bg-card border-y border-border" aria-label="Cardápio completo">
          <div className="max-w-6xl mx-auto px-4 lg:px-6 py-8 md:py-14">
            <div className="mb-5 md:mb-8 md:text-center">
              <span className="text-primary block mb-0.5 text-sm font-medium">Nosso Cardápio</span>
        ...
        {/* ── Contato — actionable info ── */}
        <section id="contato" className="bg-card border-t border-border" aria-label="Contato e localização">
          <div className="max-w-6xl mx-auto px-4 lg:px-6 py-8 md:py-14">
            <div className="mb-5 md:mb-8 md:text-center">
              <span className="text-primary block mb-0.5 text-sm font-medium">Venha nos visitar</span>
              <h2 className="text-foreground">Contato</h2>
            </div>

            <div className="flex flex-col gap-6 md:grid md:grid-cols-2">
              <div className="flex flex-col gap-3">
                <address className="bg-background border border-border rounded-lg p-4 not-italic">
                  <span className="text-muted-foreground block mb-1 text-sm">Endereço</span>
                  <p className="text-card-foreground">Rua Santa Leopoldina, nº 725</p>
                  <p className="text-card-foreground">Centro, Piúma-ES</p>
                  <span className="text-muted-foreground block mt-1 text-sm">Referência: Praça das Garças, em frente ao Bar do Bagre</span>
                </address>

                <div className="bg-background border border-border rounded-lg p-4">
                  <span className="text-muted-foreground block mb-1 text-sm">Funcionamento</span>
                  <p className="text-card-foreground"><time>Terça a Sexta: 15h às 20h</time></p>
                  <p className="text-card-foreground"><time>Sábado: 14h às 18h</time></p>
                  <span className="text-muted-foreground block mt-1 text-sm">Dom e Seg: Fechado</span>
                </div>

                <div className="bg-background border border-border rounded-lg p-4">
                  <span className="text-muted-foreground block mb-1 text-sm">Contato</span>
              ...
              <div className="overflow-hidden rounded-lg border border-border bg-muted aspect-[4/3] md:aspect-auto min-h-[320px]">
                <iframe
                  title="Mapa da localização da Docelícia Doceria e Café em Piúma-ES"
      ...
      <footer className="bg-foreground">
        <div className="max-w-6xl mx-auto px-4 lg:px-6 py-6">
          <div className="flex flex-col items-center gap-2 sm:flex-row sm:justify-between">
            <p className="text-primary-foreground font-semibold">Docelícia Doceria &amp; Café</p>
            <small className="text-primary-foreground/60 text-center">
              © 2026 Docelícia Doceria &amp; Café — Piúma-ES. Todos os direitos reservados.
            </small>
          </div>
        </div>
      </footer>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur border-t border-border safe-area-bottom" aria-label="Navegação mobile">
